...
assert.match(index, /id="verba-deep-custom-translator-enabled"/);
assert.match(index, /data-verba-deep-custom-translator-key/);
...
assert.doesNotMatch(classifier, /requestSegments|sendWithRetry|speaker-attribution/);
const protectedRepair = index.slice(
    index.indexOf('async function repairProtectedTokenIntegrity('),
    index.indexOf('function normalizedNumberTokens('),
);
assert.doesNotMatch(protectedRepair, /requestSegments|repairSegmentsByOutputScope|protected-token-repair/);

console.log('PASS: general-mode custom translator edits every AI request category with runtime variables and locked parser/token contracts; speaker attribution and protected repair are local-only.');
